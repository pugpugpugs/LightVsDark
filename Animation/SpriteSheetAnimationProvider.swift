...
protocol AnimationProviderProtocol {
    ...
    func frames(for state: State) -> [SKTexture]
}

class SpriteSheetAnimationProvider<State: Hashable>: AnimationProviderProtocol {
    
    private var stateFrames: [State: [SKTexture]] = [:]
    
    /// Initialize from a single sprite sheet with row mapping
    ...
    /// Initialize from separate textures per state
    init(stateSheets: [State: SKTexture],
         rowsPerSheet: [State: Int],
         columnsPerSheet: [State: Int] = [:]) {
